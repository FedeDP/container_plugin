...
static uint64_t reader_get_table_size(ss_plugin_table_t* t) {
    auto containers = static_cast<std::unordered_map<std::string, std::shared_ptr<const container_info>>*>(t);
    return containers->size();
}

static ss_plugin_table_entry_t *reader_get_table_entry(ss_plugin_table_t* t, const ss_plugin_state_data* key) {
    auto containers = static_cast<std::unordered_map<std::string, std::shared_ptr<const container_info>>*>(t);
...
static void reader_release_table_entry(ss_plugin_table_t* t, ss_plugin_table_entry_t* e) {
    // Unsupported for now.
}

static ss_plugin_bool reader_iterate_entries(ss_plugin_table_t* t,
...
static const ss_plugin_table_fieldinfo *list_table_fields(ss_plugin_table_t* t, uint32_t* nfields) {
    *nfields = fields.size();
    return fields.data();
}
...
static ss_plugin_rc erase_table_entry(ss_plugin_table_t* t, const ss_plugin_state_data* key) {
  	auto containers = static_cast<std::unordered_map<std::string, std::shared_ptr<const container_info>>*>(t);
    if (containers->count(key->str) == 0) {
      	return SS_PLUGIN_FAILURE;
    }
...
static void destroy_table_entry(ss_plugin_table_t* t, ss_plugin_table_entry_t* e) {
  	// Unsupported for now.
}
