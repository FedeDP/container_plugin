...
bool libvirt_lxc::resolve(const std::string& cgroup, std::string& container_id) {
    ...
    if(pos != std::string::npos) {
        ...
        if(pos2 != std::string::npos && pos2 == cgroup.length() - delimiter.length()) {
            container_id = cgroup.substr(pos + sizeof("-lxc\\x2"), pos2 - pos - sizeof("-lxc\\x2"));
            return true;
        }
    }
...
std::shared_ptr<container_info> libvirt_lxc::to_container(const std::string& container_id) {
