...
std::vector<falcosecurity::field_info> my_plugin::get_fields() {
    ...
    const falcosecurity::field_info fields[] = {
            ...
            {ft::FTYPE_BOOL, "proc.is_container_liveness_probe", "Process Is Container Liveness",
                    "'true' if this process is running as a part of the container's liveness probe."},
            {ft::FTYPE_BOOL, "proc.is_container_readiness_probe", "Process Is Container Readiness",
...
bool my_plugin::extract(const falcosecurity::extract_fields_input& in) {
    ...
    // NOTE: empty in case we are extracting from an event generated by us.
    // This means that any request to extract from it (eg: TYPE_CONTAINER_DURATION)
    // will throw an exception and MUST be managed.
    falcosecurity::table_entry thread_entry;

    // If it is an async event, try to understand whether it is a `container` async event
    if (evt_reader.get_type() == PPME_ASYNCEVENT_E) {
        falcosecurity::events::asyncevent_e_decoder ad(evt_reader);
        is_container_async_event = std::strcmp(ad.get_name(), ASYNC_EVENT_NAME_ADDED) == 0;
    }
    ...
    if (evt_reader.get_type() == PPME_CONTAINER_E ||
        ...
        // We just generated a container and we are asked to parse from it; use it.
        ...
        try {
            ...
            SPDLOG_DEBUG("cannot extract the container_id for the thread id '{}': {}", thread_id, e.what());
            return false;
        }
